...
class SafetyManager:
    ...
    def _estimate_distance(self, class_name: str, bbox_height: int) -> Tuple[float, str]:
        ...
        """
        if bbox_height <= 0:
            return (self._max_distance, 'estimated')  # Default to far away
        
        # Check for calibrated reference
        if class_name in self._reference_objects:
            ref = self._reference_objects[class_name]
            distance = (ref['height_m'] * ref['ref_distance_m'] * ref['ref_bbox_height']) / bbox_height
            return (round(max(self._min_distance, min(distance, self._max_distance)), 1), 'calibrated')
        
        # Use fallback estimation
        if self._fallback_enabled:
            distance = (self._fallback_height_m * self._fallback_ref_distance * self._fallback_ref_bbox) / bbox_height
            return (round(max(self._min_distance, min(distance, self._max_distance)), 1), 'estimated')
